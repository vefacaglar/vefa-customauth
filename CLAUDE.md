...
6. **EF Core stores** — `CustomAuthDbContext`, entity configurations, and all EF-backed store implementations are in `Vefa.CustomAuth.EntityFrameworkCore`. Registration is available via `AddCustomAuthEntityFrameworkCore(optionsAction)` and `AddCustomAuthStores<TContext>()`. `EfCustomAuthSigningKeyStore.GetAllAsync()` orders in memory to avoid SQLite `DateTimeOffset` ordering limitations.
...
16. **Security audit + P0 remediation (2026-05-28)** — A full RFC 6749 / 7009 / 7636 / 8414 / 9700 / OIDC Core 1.0 audit was run against the v1.0 implementation. The complete findings, severity-ordered remediation roadmap (21 numbered items grouped P0/P1/P2), and verification plan live at `/Users/vefa/.claude/plans/first-you-need-handover-federated-nygaard.md` — read it before touching protocol code. **All six P0 fixes shipped on `development`:**
    ...
    5. **CSRF protection on `/login` POST.** `LoginEndpointService` depends on `IAntiforgery`; the render path embeds the token in the form, the POST path calls `ValidateRequestAsync` and returns 400 on failure (with a refreshed form). `AddCustomAuth` registers `AddAntiforgery()`.
...
20. **`AddCustomAuthClient` relying-party helper (2026-05-28)** — `Vefa.CustomAuth.AspNetCore` now also ships the client-side OIDC integration so relying-party apps don't have to hand-wire `AddAuthentication().AddCookie().AddOpenIdConnect(...)` themselves:
    ...
    - **Package alignment landmine fixed**: `Microsoft.AspNetCore.Authentication.OpenIdConnect 8.0.10` transitively brings `Microsoft.IdentityModel.Protocols(.OpenIdConnect) 7.1.2`, but `Vefa.CustomAuth.Tokens` already pulls `Microsoft.IdentityModel.Tokens` / `JsonWebTokens` 8.18.0. The mixed 7.x/8.x graph silently broke OIDC discovery parsing in RP apps — `OpenIdConnectConfiguration.AuthorizationEndpoint` came back empty even though `/.well-known/openid-configuration` returned the field correctly, leading to "Cannot redirect to the authorization endpoint, the configuration may be missing or invalid." `AspNetCore.csproj` now explicitly pins `Microsoft.IdentityModel.Protocols` and `Microsoft.IdentityModel.Protocols.OpenIdConnect` at 8.18.0 with an inline comment explaining why.

21. **Post-audit hardening round 2 (2026-08-08)** — a follow-up security review of the code added after the original audit (grant handler refactor, client credentials, private_key_jwt, signing certificate) fixed four gaps (plan.md §20):
    1. **`auth_time` correctness**: `CustomAuthAuthorizationCode.AuthTime` (nullable) is set from `session.CreatedAt` at `/authorize` and emitted as the ID token `auth_time` claim; falls back to `code.CreatedAt` for pre-upgrade codes. EF hosts need a migration (see `docs/schema-changelog.md` v1.2).
    2. **`/connect/revoke` client authentication**: `RevocationEndpointService` now calls `ClientAuthenticationService.AuthenticateAsync`, so `private_key_jwt` clients must present a valid assertion (RFC 7009 §2.1).
    3. **Authorization code reuse revocation**: new `ICustomAuthTokenManager.HandleAuthorizationCodeReuseAsync` revokes the session-bound refresh token chain and writes an `AuthorizationCodeReuseDetected` audit log; called from both the consumed-code and CAS-lost paths in `AuthorizationCodeGrantHandler` (RFC 6749 §4.1.2).
    4. **Client assertion lifetime cap**: `CustomAuthOptions.ClientAssertionMaxLifetime` (default 5 min, validated > 0) rejects assertions whose `exp` exceeds now + max + clock skew; `ClientAssertionValidator` now takes a `TimeProvider`.
    Open protocol gap recorded as plan.md §21 — closed by item 22 below.
22. **Resource / Audience model (2026-08-08)** — plan.md §21 is now completed:
    1. **Scope→audience mapping**: `CustomAuthScope.Audience` (nullable, EF max 512) names the API a scope belongs to. New scoped service `Services/TokenAudienceResolver` resolves granted scope names → distinct audiences via `ICustomAuthScopeManager`. `TokenIssueRequest.Audiences` + `JwtTokenIssuer` emit them in the access token `aud` (via `SecurityTokenDescriptor.Audiences`); fallback `aud = client_id` when empty, ID token `aud` unchanged.
    2. **RFC 8707 resource indicators**: `/connect/authorize` validates `resource` params (absolute URI, no fragment, must match scope-mapped audiences; `invalid_target` redirect errors; persisted space-delimited on `CustomAuthAuthorizationCode.Resources`). All three grant handlers (via `GrantHandlerBase.ParseRequestedResources`/`TryResolveGrantAudiences`/`GetAllowedAudiencesAsync`) allow narrowing only; violations return 400 `invalid_target` *before* code consumption / token rotation. `CustomAuthRefreshToken.Resources` persists across rotation. `GrantHandlerBase` ctor now takes `TokenAudienceResolver` (breaking for custom handlers deriving from it).
    3. **Admin UI**: scope editor has an Audience field (admin.js scopeForm + index.html modal), verified live in the browser.
    4. **Schema v1.3** (`docs/schema-changelog.md`): `CustomAuthScopes.Audience`, `CustomAuthAuthorizationCodes.Resources`, `CustomAuthRefreshTokens.Resources`. New docs page `docs/flows/resource-indicators.mdx` (+ sidebar entry).
    Tests: 110 AspNetCore + 29 store/manager (13 Mongo tests require Docker); new coverage: scope-mapped aud, narrowing, invalid_target without burning the code, refresh preservation, client_credentials resources, EF/Mongo round-trips.

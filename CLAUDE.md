...
21. **Post-audit hardening round 2 (2026-08-08)** — a follow-up security review of the code added after the original audit (grant handler refactor, client credentials, private_key_jwt, signing certificate) fixed four gaps (plan.md §20):
    ...
    Open protocol gap recorded as plan.md §21: access tokens still carry `aud = client_id` — a resource/audience model (RFC 8707) is the main remaining design item. Tests: 104 AspNetCore + 28 store/manager (13 Mongo tests require Docker).
